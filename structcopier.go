...
func process(dst interface{}, src interface{}, args ...Options) error {
	...
	// check if exist filter fields
	var filterFieldsI = options.Filter["fields"]
	if filterFieldsI != nil {
		filterFields, ok := filterFieldsI.([]string)
		if (ok) {
			for i := 0; i < len(srcFieldNames); i++ {
				if contains(filterFields, srcFieldNames[i]) {
					srcFieldNames = append(srcFieldNames[:i], srcFieldNames[i+1:]...)
					i--
				}
			}
		}
